...
Game::Game() {
  shdr = new Shader("/home/mohamedmedhat/Desktop/StrawEngine/build/frag.fs",
                    "/home/mohamedmedhat/Desktop/StrawEngine/build/vert.vs");
  screenShdr =
...
void Game::InnerInit() {
  ...
  //                     CEGUI::Event::Subscriber([](const CEGUI::EventArgs& e) {std::cout << "CLICKDE" << std::endl; return true;}));
  renderer.Init();
  shdr->Init();
    ...
    for(int f =0; f < shdrBatches[i]->mySprites.size(); f++){
      if( shdrBatches[i]->mySprites[f]){
        for (int y = 0;
          ...
          shdrBatches[i]->mySprites[f]->owner->m_Addons[y]->OnUpdate(dt, alpha);
        }
        if (shdrBatches[i]->mySprites[f])
